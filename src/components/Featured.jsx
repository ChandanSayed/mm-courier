...
const Featured = () => {
  ...
  return (
    <div className="bg-blue-700 py-8">
      <h1 className="mb-8 text-2xl lg:text-5xl font-bold text-white text-center">What we do!</h1>
      <div className="flex flex-col md:flex-row gap-2.5 sm:gap-5 px-4 pb-10 max-w-[1320px] mx-auto">
        {steps.map((step, i) => {
          return (
            <div className={`bg-white rounded-[20px] md:w-[260px] p-5 sm:p-[30px] cursor-pointer ${index === i ? 'lg:flex-1' : ' bg-opacity-20 text-center'}`} key={step.id} onMouseEnter={() => handleHoverFunction(i)}>
